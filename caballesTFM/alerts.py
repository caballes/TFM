...
#Se configuran las conexciones con la base de datos
...
#Se seleccionan los países a filtrar para extraer los brotes, en este caso los europeos
european_countries = ['ALB', 'DEU', 'AND', 'AUT', 'BEL', 'BLR', 'BIH', 'BGR',
                      'CYP', 'HRV', 'DNK', 'SVK', 'SVN', 'ESP', 'EST', 'FIN',
                      'FRA', 'GRC', 'HUN', 'IRL', 'ISL', 'ITA', 'LVA', 'LIE',
                      'LTU', 'LUX', 'MKD', 'MLT', 'MDA', 'MCO', 'MNE', 'NOR',
                      'NLD', 'POL', 'PRT', 'GBR', 'CZE', 'ROU', 'RUS', 'SMR',
                      'SRB', 'SWE', 'CHE', 'UKR', 'VAT']
...
def map_alerts(days):
    ...
    #Se crea un objeto de tipo mapa
    map = folium.Map(location=[47.9600202, 9.9153559],
                     tiles='Stamen Terrain',
                     zoom_start=5)

    #Se configuran diferentes colores para las migracioens de diferentes especies
    mapcolors={
       'MIGRA1340': '#04B404',
       'MIGRA1610': '#FE2E64',
    }
    ...
    for country in european_countries:
        data_outbreaks = outbreaks.find({"country": country,
                                        "start":{"$gte": start, "$lte":end }})
                                        
        # Para cada brote se crea su objeto en el mapa
        for data in data_outbreaks:
            #Lo primero es comprobar si tiene información suficiente para representarlo...
            ...
            message="<b>Start:</b> {} <br> <b>Status:</b> {} <br> <b>End:</b> {}".format(start_date,
                                                                                         data['status'],
                                                                                         end_date)
            popup_message = """
            <p><b>Disease:</b> {}<br>
            ...
            #Ahora se comprueba en el grafo si existen rutas desde la región del brote...
            #... Para ello se crea la query ...
            ...
            #... Y se ejecuta en Neo4j ...
            migrations = graph.run(query).data()
            origin_coordinates_f = [0, 0]
            destiny_coordinates_f = [0, 0]
            
            #Para cada resultado (posibles alertas) se generan objetos para insertar en el mapa
    ...
    # Por último se añade la leyenda...
    legend_html = '''
    ...
    # ...Y se guarda el mapa renderizado
